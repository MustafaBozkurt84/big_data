...
## passwordless login to shell
- create a file that saves the password
In the file format should be:
`hostname:port:database:username:password`  
```
[train@localhost git]$ echo "localhost:5432:traindb:train:Ankara06" > ~/.pgpass
```
- change permissions of pgpass file 
`[train@localhost git]$ chmod 600 ~/.pgpass`

- connect without password promt
```
[train@localhost git]$ psql -h localhost -U train -d traindb
psql (9.2.24, server 10.13)
WARNING: psql version 9.2, server version 10.0.
         Some psql features might not work.
Type "help" for help.
